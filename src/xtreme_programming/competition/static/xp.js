// Generated by CoffeeScript 1.10.0
(function() {
  var bindChallengeModals, cycleBackground, init, setState, updateChals, updateProgress;

  $(function() {
    return init();
  });

  init = function() {
    bindChallengeModals();
    $.get("/start");
    return setInterval(updateChals, 5000);
  };

  bindChallengeModals = function() {
    return $(".chal-wrapper").click(function() {
      var chalId;
      chalId = $(this).data("id");
      return $.get("/problem/" + chalId, function(data) {
        $("#chal-problem-wrapper").html(data);
        return $("#submission_form").ajaxForm();
      });
    });
  };

  updateChals = function() {
    return $.get("/update", function(data) {
      var chalWrapper, i, len, ref, results;
      window.chals = data;
      ref = $(".chal-wrapper");
      results = [];
      for (i = 0, len = ref.length; i < len; i++) {
        chalWrapper = ref[i];
        results.push(updateProgress(chalWrapper));
      }
      return results;
    });
  };

  updateProgress = function(chal) {
    var chalEnd, chalId, chalLength, percent, progressBar, time, timeLeft;
    chalId = $(chal).data("id");
    chalLength = $(chal).data("length");
    chalEnd = window.chals[chalId]["end"];
    progressBar = $(chal).find(".progress-bar");
    if (!chalEnd) {
      progressBar.css("width", "0%");
      return;
    }
    time = new Date().getTime();
    timeLeft = chalEnd - time;
    percent = timeLeft / (chalLength * 60 * 1000) * 100;
    if (percent <= -10) {
      return;
    }
    if (percent <= 15) {
      setState(progressBar, "danger");
    } else if (percent <= 50) {
      setState(progressBar, "warning");
    }
    return $(chal).find(".progress-bar").css("width", percent + "%");
  };

  setState = function(progressBar, state) {
    $(progressBar).removeClass("progress-bar-info");
    $(progressBar).removeClass("progress-bar-warning");
    $(progressBar).removeClass("progress-bar-danger");
    return $(progressBar).addClass("progress-bar-" + state);
  };

  cycleBackground = function() {
    if (document.body.style.getPropertyValue("background-color") === "green") {
      return document.body.style.background = "blue";
    } else if (document.body.style.getPropertyValue("background-color") === "blue") {
      return document.body.style.background = "red";
    } else if (document.body.style.getPropertyValue("background-color") === "red") {
      return document.body.style.background = "purple";
    } else if (document.body.style.getPropertyValue("background-color") === "purple") {
      return document.body.style.background = "yellow";
    } else if (document.body.style.getPropertyValue("background-color") === "yellow") {
      return document.body.style.background = "white";
    } else if (document.body.style.getPropertyValue("background-color") === "white") {
      return document.body.style.background = "black";
    } else {
      return document.body.style.background = "green";
    }
  };

}).call(this);

//# sourceMappingURL=xp.js.map
